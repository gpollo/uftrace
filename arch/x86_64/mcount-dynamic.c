...
static int save_instructions(void* addr, unsigned int len) {
	...
	saved_instructions_t *insns = malloc(sizeof(unsigned int) + sizeof(uint8_t) * len);
	if (insns == NULL) {
		return -1;
	}
 ...
 * instruction. It is used in the SIGUSR1 handler of a thread during the
...
static void sigusr1_handler(int sig, siginfo_t *info, void *ucontext) {
	...
	void* trampoline_insn = get_move_instruction_address(next_insn);
