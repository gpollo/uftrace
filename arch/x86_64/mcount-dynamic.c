...
struct saved_instructions {
	unsigned int len;
	uint8_t insns[];
};

/* contains original instructions (function address -> saved_instructions) */
...
static int save_instructions(void* addr, unsigned int len) {
	...
	struct saved_instructions *insns = malloc(sizeof(unsigned int) + sizeof(uint8_t) * len);
	if (insns == NULL) {
		return -1;
	}
...
static int restore_saved_instructions(void* addr, unsigned int offset, unsigned int count) {
	struct saved_instructions *insns = hashmap_get(saved_instructions_hmap, addr);
	if (insns == NULL) {
		return -1;
	}
...
static int save_int3_mdi(void *address, void *return_address, struct mcount_dynamic_info *mdi) {
	...
	int3_patch_t *patch = malloc(sizeof(*patch));
	if (patch == NULL) {
		return -1;
	}
...
static void sigtrap_handler(int sig, siginfo_t *info, void *ucontext) {
	...
	void* int3_address = (void*) mctx->gregs[REG_RIP] - 1;
	int3_patch_t *patch = hashmap_get(int3_patchs_hmap, int3_address);

	uint64_t return_address = (uint64_t) patch->return_address;

	mctx->gregs[REG_RIP] = patch->mdi->trampoline;
	mctx->gregs[REG_RSP] -= 8;
	memcpy((void*) mctx->gregs[REG_RSP], &return_address, 8);

	//printf("int3 address   = %p\n", int3_address);
...
/* (emulated call return address -> real call return address) */
...
/* (instruction address -> instruction address in trampoline) */
static struct Hashmap *insn_to_trampoline_insn_hmap;

static int map_insn_range_to_trampoline_insns(void* insns, void* trampoline_insns, unsigned int count) {
	if (insn_to_trampoline_insn_hmap == NULL) {
		insn_to_trampoline_insn_hmap = hashmap_create(4, hashmap_ptr_hash, hashmap_ptr_equals);
	}

	for (unsigned int i = 0; i < count; i++) {
		if (hashmap_contains_key(insn_to_trampoline_insn_hmap, insns + i)) {
			continue;
		}

		if (hashmap_put(insn_to_trampoline_insn_hmap, insns + i, trampoline_insns + i) == NULL) {
			return -1;
		}
	}
...
static int unmap_insn_range_to_trampoline_insns(void* insns, void* trampoline_insns, unsigned int count) {
	if (insn_to_trampoline_insn_hmap == NULL) {
		insn_to_trampoline_insn_hmap = hashmap_create(4, hashmap_ptr_hash, hashmap_ptr_equals);
	}
	...
	for (unsigned int i = 0; i < count; i++) {
		...
		if (hashmap_remove(insn_to_trampoline_insn_hmap, insns + i)) {
			return -1;
		}
	}
...
static void* get_trampoline_address_of_instruction(void* insn) {
	if (insn_to_trampoline_insn_hmap == NULL) {
		return NULL;
	}

	return hashmap_get(insn_to_trampoline_insn_hmap, insn);
}
...
static void sigusr1_handler(int sig, siginfo_t *info, void *ucontext) {
	...
	void* next_insn = (void*) mctx->gregs[REG_RIP];
	void* trampoline_insn = get_trampoline_address_of_instruction(next_insn);
	if (trampoline_insn == NULL) {
		//printf("SIGUSR1 handler exited\n");
		return;
	}

	printf("moving thread to trampoline: %p -> %p\n", next_insn, trampoline_insn);
	mctx->gregs[REG_RIP] = (uint64_t) trampoline_insn;
}

static int send_sigusr1_to_all_threads() {
	errno = 0;
	...
	while (1) {
		...
		long tid = strtol(directory_entry->d_name, NULL, 10);
		if (errno != 0 || tid < 0) {
...
static bool sigusr1_handler_configured = false;

static int configure_sigusr1_handler(void) {
	if (sigusr1_handler_configured) {
		return 0;
	}
    ...
    if (sigaction(SIGUSR1, &act, NULL) < 0) {
		pr_err("failed to configure SIGUSR1 handler\n");
        return -1;
    }

	pr_dbg("configured SIGUSR1 handler\n");
	sigusr1_handler_configured = true;
...
int mcount_setup_trampoline(struct mcount_dynamic_info *mdi)
{
	...
	if (configure_sigusr1_handler() < 0) {
		return -1;
	}
...
static void patch_code(struct mcount_dynamic_info *mdi,
		       uintptr_t addr, uint32_t original_code_size)
{	
	/*
	 ...
	 *
	 * The method used to move the threads is to send the SIGUSR1
	...
	if (map_insn_range_to_trampoline_insns(original_code_addr, modified_code_addr, original_code_size) < 0) {
		pr_dbg("failed to map instructions to trampoline\n");
	}

	send_sigusr1_to_all_threads();

	if (unmap_insn_range_to_trampoline_insns(original_code_addr, modified_code_addr, original_code_size) < 0) {
...
static int unpatch_normal_func(struct mcount_dynamic_info *mdi, struct sym *sym)
{
	 ...
	 * executing in the modified instructions to the modified instructions
	...
	if (map_insn_range_to_trampoline_insns(modified_code_addr, original_code_addr, original_code_size) < 0) {
		pr_dbg("failed to map instructions to trampoline\n");
	}

	send_sigusr1_to_all_threads();

	if (unmap_insn_range_to_trampoline_insns(modified_code_addr, original_code_addr, original_code_size) < 0) {
		pr_dbg("failed to unmap instructions to trampoline\n");
	}
